...
interface SkillItemModalConfig extends BaseModalConfig {
    description: string;
 ...
 */
export default class SkillItemModal extends BaseModal {
    
    constructor(scene: SceneWithRexUI, config: SkillItemModalConfig) {
        super(scene, config);
    }
    ...
    protected createDialog(config: SkillItemModalConfig): Dialog {
    ...
    protected createModalContent(config: SkillItemModalConfig) {
        ...
        modalContent.add(UIFactory.createButtonRex(this.scene, {
            text: config.removeButtonText ?? "Level down",
            buttonSize: "large",
        }).onClick((click, gameObject, pointer) => {
            if(config.removeButtonOnClick !== undefined)
