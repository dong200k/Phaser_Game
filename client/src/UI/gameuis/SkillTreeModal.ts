import { Dialog, RoundRectangle, ScrollablePanel, Sizer } from "phaser3-rex-plugins/templates/ui/ui-components";
import BaseModal, { BaseModalConfig } from "../modals/BaseModal";
import { ColorStyle } from "../../config";
import UIPlugins from "phaser3-rex-plugins/templates/ui/ui-plugin";
import ClientFirebaseConnection from "../../firebase/ClientFirebaseConnection";
import type SkillData from "../../../../server/src/rooms/game_room/schemas/Trees/Node/Data/SkillData";
import type Node from "../../../../server/src/rooms/game_room/schemas/Trees/Node/Node";
import RexUIBase, { SceneWithRexUI } from "../RexUIBase";
import PlayerService from "../../services/PlayerService";
import UIFactory from "../UIFactory";
import ConfirmModal from "../modals/ConfirmModal";
import SkillItemModal from "../modals/SkillItemModal";
import ButtonRex from "../ButtonRex";
...
export default class SkillTreeModal extends RexUIBase {
    ...
    public showSkillTreeModal() {
        // Clean up old popup.
        if(this.popup) {
            this.popup.destroy();
        }
        // Initialize variables.
        let closeButton: ButtonRex | undefined;
        // Create popup.
        this.popup = this.rexUI.add.dialog({
            background: this.rexUI.add.roundRectangle(0, 0, 100, 100, 5, ColorStyle.primary.hex[900]),
            title: closeButton = UIFactory.createButtonRex(this.scene, {text: "Exit Shop"}),
    ...
    private createModalContent() {
        ...
        let scrollablePanel = this.rexUI.add.scrollablePanel({
            width: this.scene.game.scale.width * (2/3),
            height: this.scene.game.scale.height * (3/5),
            panel: {
    ...
    private createOrUpdatePanel() {
        if(this.panelSizer === undefined) {
            ...
            // ----- Title ------
            let title = UIFactory.createTextBoxPhaser(this.scene, "Skills", "h3");
    ...
    private createButton(text: string){
        ...
        textSizer.add(UIFactory.createTextBoxPhaser(this.scene, text, "l4"));
        sizer.add(textSizer)
    ...
    private createSkillItem(skillItem: SkillItem) {
        ...
        textSizer.add(UIFactory.createTextBoxPhaser(this.scene, skillItem.name.toUpperCase(), "l4").setWordWrapWidth(220).setAlign("left"));
        textSizer.addNewLine();
        textSizer.add(UIFactory.createTextBoxPhaser(this.scene, `${skillItem.currentLevel === 0 ? "None" : skillItem.levels[skillItem.currentLevel - 1].value}` , "l4"));
        if(skillItem.currentLevel < skillItem.levels.length)
            textSizer.add(UIFactory.createTextBoxPhaser(this.scene, ` -> ${skillItem.levels[skillItem.currentLevel].value}`, "l4").setColor(ColorStyle.neutrals[200]));
        else 
            textSizer.add(UIFactory.createTextBoxPhaser(this.scene, ` MAXED`, "l4").setColor(ColorStyle.neutrals[200]));
        ...
        verticalSizer.add(sizer);
        if(skillItem.currentLevel < skillItem.levels.length)
            verticalSizer.add(UIFactory.createTextBoxPhaser(this.scene, `${skillItem.levels[skillItem.currentLevel].cost} Coins`, "l4"));
