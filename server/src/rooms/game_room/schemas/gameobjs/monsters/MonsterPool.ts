...
/** MonsterPool is used to store reuseable Monster objects. */
export default class MonsterPool {

    private poolMap: Map<string, ObjectPool<Monster>> = new Map()
    ...
    public getInstance(type: string){
        let pool = this.poolMap.get(type) as ObjectPool<Monster>;
     ...
     */
    public returnInstance(type: string, instance: Monster){
        let pool = this.poolMap.get(type) as ObjectPool<Monster>;
    ...
    public addPoolType(type: string){
        ...
        let pool = new ObjectPool<Monster>(prototype as Monster)
