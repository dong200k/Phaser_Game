...
export default class Attack extends StateNode {
    ...
    public onEnter(): void {
        ...
        if(stateMachine.isTransformed()) monster.animation.playAnimation("attack")
    }
    ...
    protected attack() {
        ...
        if(stateMachine.isTransformed()) this.attackDemonSlime()
        else this.attackNormal()
    }

    protected attackNormal() {
        let stateMachine = this.getStateMachine<DemonSlimeController>();
        let monster = stateMachine.getMonster();
        let target = monster.getAggroTarget();
        ...
        if(target) {
            ...
            projectileConfig = {
                ...
                activeTime: 1000,
                poolType: "demon_slime_normal_monster_projectile",
                attackMultiplier: 1,
                magicMultiplier: 0,
